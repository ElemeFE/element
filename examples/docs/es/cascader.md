...
### Contenido de nodo de búsqueda personalizado

Posibilidad de personalizar el contenido del nodo de las alternativas filtradas

:::demo El contenido del nodo de las alternativas después de que el selector de cascada haya activado el filtro     `filterable` se puede personalizar con `scoped slot`. Tendrá acceso a `node` y `data` en el ámbito de aplicación, representando el objeto Node y los datos del nodo actual respectivamente.
```html
<el-cascader :options="options" filterable>
  <template slot="label" slot-scope="{ node, data }">
...
| label | el contenido personalizado del nodo cascader (filterable), el parámetro es { node, data }, que son el actual objeto Node y los datos del nodo respectivamente. |
