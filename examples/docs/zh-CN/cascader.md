      ...
      return {
        props: {
          ...
          lazyLoad (node, resolve) {
            ...
            if (node.level < 3) {
              setTimeout(() => {
                const nodes = Array.from({ length: node.level + 1 })
                  .map(item => ({ value: ++id, label: `选项${id}` }))
                resolve(nodes)
              }, 1000)
            } else {
...
级联面板是级联选择器的核心组件，与级联选择器一样，有单选、多选、冬天加载等多种功能。
...
| 参数      | 说明    | 类型      | 可选值       | 默认值   |
|---------- |-------- |---------- |-------------  |-------- |
| value / v-model | 选中项绑定值 | array / string / number | — | — |
