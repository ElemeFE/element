...
可以内容超出时选中项前面实现省略号，方便查看最末级路径。

:::demo 属性`before-ellipsis`定义了是否在选中项前面实现省略号，将其赋值为`true`则在超出前端显示省略号

```html
<el-cascader :options="options" :before-ellipsis="true"></el-cascader>
<script>
  export default {
    data() {
      return {
