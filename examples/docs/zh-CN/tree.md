...
| check          | 当复选框被点击的时候触发 | 共三个参数，依次为：传递给 `data` 属性的数组中该节点所对应的对象、树目前的选中状态对象，包含 checkedNodes、checkedKeys、halfCheckedNodes、halfCheckedKeys 四个属性， 当前节点到根节点路径上的所有节点数据（当without-chilren为true和node-key有值时，否则为空数组）|
