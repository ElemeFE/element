...
<style>
  .tm-table .warning-row {
    background: oldlace;
  }

  .tm-table .success-row {
    background: #f0f9eb;
  }
...
:::demo After setting attribute `data` of `tm-table` with an object array, you can use `prop` (corresponding to a key of the object in `data` array) in `tm-table-column` to insert data to table columns, and set the attribute `label` to define the column name. You can also use the attribute `width` to define the width of columns.
...
:::demo Use `row-class-name` in `tm-table` to add custom classes to a certain row. Then you can style it with custom classes.
```html
<template>
  <tm-table
    :data="tableData2"
...
:::demo By setting the attribute `height` of `tm-table`, you can fix the table header without any other codes.
```html
<template>
  <tm-table
    :data="tableData3"
...
:::demo Attribute `fixed` is used in `tm-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
```html
<template>
  <tm-table
    :data="tableData"
    ...
    <tm-table-column
      ...
      <template slot-scope="scope">
        <tm-button @click="handleClick" type="text" size="small">Detail</tm-button>
        <tm-button type="text" size="small">Edit</tm-button>
      </template>
    </tm-table-column>
  </tm-table>
</template>
...
:::demo  By setting the attribute `max-height` of `tm-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
```html
<template>
  <tm-table
    :data="tableData4"
...
:::demo Only need to place tm-table-column inside a tm-table-column, you can achieve group header.
```html
<template>
  <tm-table
    :data="tableData3"
    ...
    <tm-table-column label="Delivery Info">
      <tm-table-column
        prop="name"
        label="Name"
        width="120">
      </tm-table-column>
      <tm-table-column label="Address Info">
        <tm-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <tm-button @click="setCurrent(tableData[1])">Select second row</tm-button>
    <tm-button @click="setCurrent()">Clear selection</tm-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <tm-button @click="toggleSelection([tableData3[1], tableData3[2]])">Toggle selection status of second and third rows</tm-button>
    <tm-button @click="toggleSelection()">Clear selection</tm-button>
  </div>
</template>
...
:::demo Set attribute `filters` and `filter-method` in `tm-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
```html
<template>
  <tm-table
    :data="tableData"
    ...
    <tm-table-column
      ...
      <template slot-scope="scope">
        <tm-tag
          ...
          close-transition>{{scope.row.tag}}</tm-tag>
      </template>
    </tm-table-column>
  </tm-table>
</template>
...
<template>
  <tm-table
    ...
    <tm-table-column
      ...
      width="180">
      <template slot-scope="scope">
        <tm-popover trigger="hover" placement="top">
          <p>Name: {{ scope.row.name }}</p>
          <p>Addr: {{ scope.row.address }}</p>
          <div slot="reference" class="name-wrapper">
            <tm-tag size="medium">{{ scope.row.name }}</tm-tag>
          </div>
        </tm-popover>
      </template>
    </tm-table-column>
    <tm-table-column
      ...
      <template slot-scope="scope">
        <tm-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</tm-button>
        <tm-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</tm-button>
      </template>
    </tm-table-column>
  </tm-table>
</template>
...
:::demo Activate expandable row by adding type="expand" and scoped slot. The template for tm-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `Scoped slot` in custom column templates.
```html
<template>
  <tm-table
    :data="tableData3"
...
:::demo To customize row indices, use `index` attribute on <tm-table-column> with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
| resizable | whether column width can be resized, works when `border` of `tm-table` is `true` | boolean | — | false |
