...
| row-key | key of row data, used for optimizing rendering. Required if `reserve-selection` is on. When the type is String, support for multi-level access, for example: ```user.info.id```, but does not support``` user.info[0].id```, in this case please use ```Function```. | Function(row)/String | — | — |
