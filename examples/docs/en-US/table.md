...
| sort-method | sorting method, works when `sortable` is `true`. Should return a number. | Function(a, b) | — | — |
| sort-by | sorting by method or property, works when `sortable` is `true` and `sort-method` is `undefined`. | Function(row, index)/string | — | — |
