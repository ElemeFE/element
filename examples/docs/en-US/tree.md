...
| getCurrentKey   | return the highlight node's key | return null if no node to be the highlight |
| getCurrentNode  | return the highlight node | return null if no node to be the highlight |
| setCurrentKey   | set node to be the highlight by key, only works when `node-key` is assigned | (key) the node's key to be the highlight |
| setCurrentNode  | set node to be the highlight, only works when `node-key` is assigned | (node) the node to be the highlight |
