...
### Content of custom search node

Ability to customize the content of the filtered alternatives node.

:::demo the content of the alternatives node after the cascade selector has activated the `filterable` filter can be customized with `scoped slot`. You'll have access to `node` and `data` in the scope,  standing for the Node object and node data of the current node respectively。
```html
...
| label | the custom content of cascader (filterable) node, the parameter is { node, data }, which are current Node object and node data respectively. |
