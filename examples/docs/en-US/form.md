...
| validateAndScroll | Similar to validate, but after validation, the first error element scrolls into the visible window. The parameter options defines the scrolling behavior. The default value is { scrollMode: 'if-needed', behavior: 'smooth', block: 'start' }. For detailed configuration, see [Configuration Parameters](https://github.com/stipsan/scroll-into-view-if-needed#options); another parameter rule is the same as the rule of the callback function in validate. If you do not pass in a callback function, a promise will be returned. The two parameters have no order, and can be passed only one or not. | Function([options: object], [callback: Function(boolean, object)])
