...
### Contenu du noeud de recherche personnalisé

Possibilité de personnaliser le contenu du noeud alternatives filtrées.

:::demo le contenu du noeud alternatives après que le sélecteur en cascade ait activé le filtre `filterable` peut être personnalisé avec `scoped slot`. Vous aurez accès à `node` et` data` dans la portée, correspondant respectivement à l'objet Node et aux données de noeud du noeud actuel.
```html
...
|---------|-------------|
| - | Le contenu personnalisé du noeud cascader, le paramètre est { node, data }, qui sont respectivement les objets de noeud et les données de noeud actuels. |
| label | Le contenu personnalisé du noeud cascader (filterable), le paramètre est { node, data }, qui sont respectivement les objets de noeud et les données de noeud actuels. |
