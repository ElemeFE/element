<template>
  <table @click="handleMonthTableClick" class="el-quarter-table">
    <tbody>
    <tr>
      <td :class="getCellStyle(1)" cellNum="1">
        <a class="cell">{{ t('el.datepicker.quarter1') }}</a>
      </td>
      <td :class="getCellStyle(2)" cellNum="2">
        <a class="cell">{{ t('el.datepicker.quarter2') }}</a>
      </td>
    </tr>
    <tr>
      <td :class="getCellStyle(3)" cellNum="3">
        <a class="cell">{{ t('el.datepicker.quarter3') }}</a>
      </td>
      <td :class="getCellStyle(4)" cellNum="4">
  ...
  export default {
    ...
    methods: {
      ...
      handleMonthTableClick(event) {
        ...
        const cell = target.parentNode.getAttribute('cellNum');
        this.$emit('pick', cell);
      }
    }
  };
