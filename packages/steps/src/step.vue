<template>
  <div class="el-step" :style="style" :class="[!isSimple && `is-${$parent.direction}`, isSimple && 'is-simple', isLast && !space && !isCenter && 'is-flex', isCenter && !isVertical && !isSimple && 'is-center']">
    <!-- icon & line -->
    <div class="el-step__head" :class="`is-${currentStatus}`">
      ...
      <div class="el-step__icon" :class="`is-${icon || isSuccess || isError ? 'icon' : 'text'}`">
        <slot name="icon">
          <i v-if="icon" class="el-step__icon-inner" :class="[icon]"></i>
          <div v-else-if="!icon && !isSimple && !isSuccess && !isError" class="el-step__icon-inner">
            {{ index + 1 }}
          </div>
          <i v-else :class="['el-icon-' + (isSuccess ? 'check' : 'close')]" class="el-step__icon-inner is-status"> </i>
        </slot>
      </div>
    </div>
    <!-- title & description -->
