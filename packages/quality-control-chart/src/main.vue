    ...
    <div class='el-quality-control-chart' :v-loading='!arrData.length && !lineBase' >
      <div class='block'>
        ...
        <div v-show='arrData.length && lineBase'>
            <div v-if="height != null" id='myEchartsContainer' :style='{height: height,width: width}'></div>
        </div>
    </div>
</template>
...
export default {
  ...
  mounted() {
    console.log('abc--->', this.width, this.height);
    this.width = this.$parent.$el.offsetWidth + 'px';
    this.height = this.$parent.$el.offsetHeight + 'px';
    console.log('abc--2->', this.width, this.height);
    setTimeout(() => {
      this.getEcharts();
    });
  },
