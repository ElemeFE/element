...
*2019-06-24*
   ...
   - Зависимости vue и vue-template-compiler обновлены до версии ~2.6.10

### 1.2.47

*2019-06-24*

- Обновлен компонент `AdvancedDatePicker`:
    - Улучшена работа ручного ввода даты (с маской);

*2019-06-17*

  - Обновлен компонент `AdvancedDatePicker`:
    - Атрибут `local-storage-date` заменен на `value`;
    - Фикс с получением даты извне;
    - Фикс очистки поля с выбранной датой.

### 1.2.45

*2019-06-13*

  - Обновлены стили компонента `AdvancedDatePicker`.

### 1.2.43

*2019-06-05*

  - Изменена сборка иконок (компонент `Icon`)

### 1.2.42

*2019-05-13*

  - Пофиксил артефакты при ховере на первый и последний итем в селекте
  - Выровнял позиционирование дропдауна относительно инпута
  - Сделал дефолтную высоту итема в дропдауне 56px
  - Сделал везде отступ дропдауна от инпута 8px
  - Поменял цвет текста заполненного инпута в дейтпикере
  - Воровнял стрелочки выбора месяца в дейтпикере
...
*2019-05-07*
  ...
  - Внесены правки по стилям `pagination`

### 1.2.41

*2019-05-07*

  - Внесены правки по стилям `date-picker`

*2019-05-02*

  - В компононент `Tabs` добавлены атрибуты `list-class` и `list-item-class`

*2019-04-30*

  - Скрываем разделитель в мультиселекте если выбран только один элемент
  - Уменьшен паддинг в мультиселекте, чтобы вмешалось больше тэгов на меньших разрешениях

### 1.2.40

*2019-04-26*

  - Добавлен пропс hide-year-header в date-picker.
