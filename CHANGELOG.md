...
- Был выявлен bug. При закрытие Messegebox, он не удаляется и при открытие нового, просто подставляется контент, но переменные остаются. Это может привести к утечки памяти. Big пофикшен, Сам компонент требует рефакторинга и изменении логики работы.
