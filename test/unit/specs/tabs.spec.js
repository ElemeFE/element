...
describe('Tabs', () => {
  ...
  it('editable', done => {
    ...
    setTimeout(() => {
      ...
      tabList[1].querySelector('.tm-icon--cross-bold').click();
      vm.$nextTick(() => {
  ...
  it('addable & closable', done => {
    ...
    setTimeout(() => {
      ...
      vm.$nextTick(() => {
        ...
        tabList[2].querySelector('.tm-icon--cross-bold').click();
        vm.$nextTick(() => {
          expect(tabList.length).to.be.equal(2);
          expect(paneList.length).to.be.equal(2);
          expect(tabList[1].classList.contains('is-active')).to.be.true;
  ...
  it('closable in tab-pane', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelectorAll('.tm-icon--cross-bold').length).to.equal(2);
      done();
    }, 100);
  });
  it('disabled', done => {
