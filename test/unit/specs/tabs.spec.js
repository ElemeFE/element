...
describe('Tabs', () => {
  ...
  it('editable', done => {
    ...
    setTimeout(_ => {
      ...
      tabList[1].querySelector('.siagri-icon-close').click();
      vm.$nextTick(_ => {
  ...
  it('addable & closable', done => {
    ...
    setTimeout(_ => {
      ...
      vm.$nextTick(_ => {
        ...
        tabList[2].querySelector('.siagri-icon-close').click();
        vm.$nextTick(_ => {
          expect(tabList.length).to.be.equal(2);
          expect(paneList.length).to.be.equal(2);
          expect(tabList[1].classList.contains('is-active')).to.be.true;
  ...
  it('closable in tab-pane', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelectorAll('.siagri-icon-close').length).to.equal(2);
      done();
    }, 100);
  });
  it('disabled', done => {
